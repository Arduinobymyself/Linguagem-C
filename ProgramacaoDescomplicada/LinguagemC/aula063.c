/*
Título:
Autor: 			Eng. Marcelo Moraes
Local: 			Sorocaba - SP
Data: 			Janeiro 2023
Descrição: 		Aulas do curso de Linguagem C ANSI
Observações:
	aula063 - Aloação Dinâmica - calloc()
	
	Função calloc(): serve para alocar memória durante a execução do programa.
	Ela faz o pedido de memória ao processador e retorna um ponteiro com o
	endereço do início o bloco de memória alocado.
	
	Protótipo:
	void* calloc(unsigned int num, unsigned int size);
	
	Funcionamento:
	A função calloc() recebe por parâmetro
	- a quantidade de elementos no array a ser alocado 
	- tamanho de cada elemento do array
	e retorna;
	- NULL, no caso de erro;
	- ponteiro para a primeria posição do array;
	
	Exemplo:
	int *v = (int*) calloc(50,4); // cria array de 50 inteiros (200 bytes)
	char *c = (char*) alloc(200,1); // cria array de 200 char (200 bytes)
	
	Na alocação da memória, deve-se levar em conta o tamanho do tipo.
	Exemplo:
	int *v = (int*) calloc(50,4); // cria array de 50 inteiros (200 bytes)
	char *c = (char*) calloc(200,1); // cria array de 200 char (200 bytes)

	Solução: usar sizeof();
	int *v = (int*) calloc(50*sizeof(int)); // cria array de 50 inteiros (200 bytes)
	char *c = (char*) calloc(50*sizeof(char)); // cria array de 50 char (50 bytes)
	
	Se não houver espaço em memória sufuciente para alocação requisitada,
	a função calloc() retorna NULL.
	Isso deve ser checado no programa
	
	int *p, i;
	p = (int *) calloc(5*sizeof(int));
	if(p == NULL){
		printf("Erro! Sem memória\n");
		exit(1);
	}
	for(i = 0; i < 5; i++){
		printf("Digite p[%d] ", i);
		scanf("%d", &p[i]);
	}
	printf("\n");
	    for(i = 0; i < 5; i++){
		printf("%d ", p[i]);
	}
	
	A Função free(); sempre que alocamos memória é necessário liberá-la
	quando ela não or mais necessária.
	
	free(p);
	
	malloc x calloc:
	Ambas tem o mesmo propósito, mas callo() inicializa todos os bits do espaço 
	alocada com 0s.
	
	int *p, *p1, i;
	p = (int *) malloc(5*sizeof(int));
	p1 = (int *) calloc(5,sizeof(int));
	
	printf("calloc \t\t malloc\n");
	for(i = 0; i < 5; i++){
		printf("p1[%d] = %d \t", i, p1[i]); // mostra todos os espaços com zeros
		printf(" p[%d] = %d\n", i, p[i]);   // pode mostrar lixo de memória
	}
	
	free(p);
	free(p1);





*/

// --- inclusão de bibliotecas --- //
#include <stdio.h>
#include <stdlib.h>
#include <locale.h>

// --- definição de parâmetros --- //

// --- estruturas e variáveis globais --- //

// --- protóritpo das funções auxiliares --- //

// --- programa principal --- //
int main(){
	setlocale(LC_ALL, "Portuguese");
	printf("\n\n");
	
	int *p, *p1, i;
	p = (int *) malloc(5*sizeof(int));
	p1 = (int *) calloc(5,sizeof(int));
	
	printf("calloc \t\t malloc\n");
	for(i = 0; i < 5; i++){
		printf("p1[%d] = %d \t", i, p1[i]);
		printf(" p[%d] = %d\n", i, p[i]);
	}
	
	free(p);
	free(p1);
	
	



	printf("\n\n");
	//system("pause");
	return 0;
}

// --- desenvolvimento das funções auxiliares --- //

